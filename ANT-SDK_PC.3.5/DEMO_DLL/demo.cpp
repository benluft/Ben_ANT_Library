...
BOOL Test_ChannelCallback(UCHAR ucChannel_, UCHAR ucEvent_)
{
   ...
   //Note should only be detected once when it is sent, these are true if the note has
   //been detected to be on at least once with no zero in between
   static BOOL bNoteOneHeld = FALSE;
   ...
   if(bPrintBuffer)
   {
      if(1) //Needs to always happen
      {
		  ...
		  if (bNoteOneHeld == TRUE)
		  {
			  if (aucChannelBuffer[ucDataOffset + 0] == 0)
			  {
				  bNoteOneHeld = FALSE;
			  }
		  }
		  ...
		  if (bNoteFourHeld == TRUE)
		  {
			  if (aucChannelBuffer[ucDataOffset + 3] == 0)
			  {
				  bNoteFourHeld = FALSE;
			  }
		  }
